...
3. Writes the remaining app, Nginx, and monitoring values to `.env` on the server with file mode `600`.

`PROD_APP_DIR` inside `DEPLOY_ENV_FILE` must point to the repository directory on the production server.

`PROD_SSH_PRIVATE_KEY` must be the private key file contents without indentation. Do not use the public key from `authorized_keys`; public keys start with `ssh-rsa` or `ssh-ed25519` and cannot authenticate the deploy job.
...
PROD_SSH_PORT=22
PROD_SSH_PRIVATE_KEY=REPLACE_WITH_PRIVATE_KEY_CONTENTS
...
GOOGLE_OAUTH_REDIRECT_URI=https://api.xn--vk1b177d.com/login/oauth2/code/google
...
Host Nginx deployment is controlled by `.env`.
...
If `DEPLOY_ENV_FILE` contains `APP_IMAGE` or `APP_IMAGE_TAG`, CI/CD ignores those values and writes the current GitHub Actions image metadata to the server `.env` file.

Deploy scripts use `docker` directly when the SSH user can access the Docker socket. If not, they automatically fall back to `sudo -n docker`, so the server user needs either docker group access or passwordless sudo for Docker.

Monitoring deployment is controlled by `.env`.
